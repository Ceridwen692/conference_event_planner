...
const ConferenceEvent = () => {
    ...
    const handleToggleItems = () => setShowItems((s) => !s);
    ...
    const handleRemoveFromCart = (index) => {
        const item = venueItems[index];
        if (!item) return;
        if (item.quantity > 0) dispatch(decrementQuantity(index));
    };
    ...
    const calculateTotalCost = (section) => {
        ...
        if (section === "venue") {
            ...
            mealsItems.forEach((item) => {
                if (item.selected) totalCost += (item.cost || 0) * (numberOfPeople || 0);
            });
        }
        return totalCost;
    };
        ...
        <>
            <navbar className="navbar_event_conference">
                ...
                <div className="left_navbar">
                    ...
                    <button className="details_button" onClick={() => setShowItems((s) => !s)}>Show Details</button>
                </div>
            </navbar>
                    ...
                    <div className="items-information">
                                    ...
                                    <div className="venue_main" key={index}>
                                        ...
                                        <div className="button_container">
                                            {item.name === "Auditorium Hall (Capacity:200)" ? (
                                                ...
                                                <div className="button_container">
                                                    <button className={item.quantity === 0 ? " btn-warning btn-disabled" : "btn-warning btn-plus"} onClick={() => handleRemoveFromCart(index)}>&#8211;</button>
                                                    <span className="selected_count">{item.quantity > 0 ? ` ${item.quantity}` : "0"}</span>
                                                    <button className={item.quantity === 10 ? " btn-success btn-disabled" : "btn-success btn-plus"} onClick={() => handleAddToCart(index)}>&#43;</button>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                        ...
                        <div id="meals" className="venue_container container_main">
                            ...
                            <div className="input-container venue_selection">
                                ...
                                <input type="number" className="input_box5" id="numberOfPeople" value={numberOfPeople} onChange={(e) => setNumberOfPeople(parseInt(e.target.value) || 1)} min="1" />
                            </div>
